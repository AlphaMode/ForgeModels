...
public class PerspectiveMapWrapper implements IDynamicBakedModel
{
    ...
    public static ImmutableMap<ItemTransforms.TransformType, Transformation> getTransforms(ModelState state)
    {
        ...
        {
            Transformation tr = ((IForgeModelState)state).getPartTransformation(type);
            if(!((IForgeTransformation)(Object)tr).isIdentity())
            {
                map.put(type, tr);
            }
        }
    ...
    public static BakedModel handlePerspective(BakedModel model, ImmutableMap<ItemTransforms.TransformType, Transformation> transforms, ItemTransforms.TransformType cameraTransformType, PoseStack mat)
    {
        ...
        if (!((IForgeTransformation)(Object)tr).isIdentity())
        {
            ((IForgeTransformation)(Object)tr).push(mat);
        }
        return model;
    }
    ...
    {
        Transformation tr = ((IForgeModelState)state).getPartTransformation(cameraTransformType);
        if (!((IForgeTransformation)(Object)tr).isIdentity())
        {
            ((IForgeTransformation)(Object)tr).push(mat);
        }
    ...
    @Override public boolean isAmbientOcclusion(BlockState state) { return ((IForgeBakedModel)parent).isAmbientOcclusion(state); }
    ...
    {
        return ((IForgeBakedModel)parent).getQuads(state, side, rand, extraData);
    }
    ...
    private class OverrideListWrapper extends ItemOverrides implements ItemOverridesEx
    {
        ...
        {
            return ((ItemOverridesEx)parent.getOverrides()).getOverrides();
        }
    }
}
