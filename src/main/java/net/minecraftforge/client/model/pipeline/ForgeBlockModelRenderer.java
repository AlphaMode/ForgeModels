...
//        else
//        {
            return this.tesselateWithoutAO(world, model, state, pos, matrixStack, buffer, checkSides, rand, seed, combinedOverlayIn, modelData);
//        }
    }
...
//        else
//        {
            return this.tesselateWithAO(world, model, state, pos, matrixStack, buffer, checkSides, rand, seed, combinedOverlayIn, modelData);
//        }
    }
    ...
    public static boolean render(VertexLighterFlat lighter, BlockAndTintGetter world, BakedModel model, BlockState state, BlockPos pos, PoseStack matrixStack, boolean checkSides, Random rand, long seed, IModelData modelData)
    {
        ...
        boolean empty = true;
        rand.setSeed(seed);
        List<BakedQuad> quads = ((IForgeBakedModel)model).getQuads(state, null, rand, modelData);
        if(!quads.isEmpty())
        {
            ...
            {
                ((IVertexProducer)quad).pipe(lighter);
            }
        }
        for(Direction side : Direction.values())
        {
            rand.setSeed(seed);
            quads = ((IForgeBakedModel)model).getQuads(state, side, rand, modelData);
            if(!quads.isEmpty())
            {
