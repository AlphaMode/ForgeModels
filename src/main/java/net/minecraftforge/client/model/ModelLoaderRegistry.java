...
//            }
    ...
    {
        BakedModel model;
        IModelGeometry<?> customModel = ((BlockModelEx)blockModel).ForgeModels$getCustomData().getCustomGeometry();
        ModelState customModelState = ((BlockModelEx)blockModel).ForgeModels$getCustomData().getCustomModelState();
        if (customModelState != null)
            modelTransform = new CompositeModelState(modelTransform, customModelState, modelTransform.isUvLocked());

        if (customModel != null)
            model = customModel.bake(((BlockModelEx)blockModel).ForgeModels$getCustomData(), modelBakery, spriteGetter, modelTransform, blockModel.getOverrides(modelBakery, otherModel, spriteGetter), modelLocation);
        else
        {
            ...
            {
                model = ((BlockModelEx)blockModel).bakeVanilla(modelBakery, otherModel, spriteGetter, modelTransform, modelLocation, guiLight3d);
            }
        }
    ...
    public static class ExpandedBlockModelDeserializer extends BlockModel.Deserializer
    {
        ...
        public BlockModel deserialize(JsonElement element, Type targetType, JsonDeserializationContext deserializationContext) throws JsonParseException {
            ...
            if (geometry != null) {
                elements.clear();
                ((BlockModelEx)model).ForgeModels$getCustomData().setCustomGeometry(geometry);
            }

            ModelState modelState = deserializeModelTransforms(deserializationContext, jsonobject);
            if (modelState != null)
            {
                ((BlockModelEx)model).ForgeModels$getCustomData().setCustomModelState(modelState);
            }
            ...
            if (jsonobject.has("visibility"))
            {
                ...
                {
                    ((BlockModelEx)model).ForgeModels$getCustomData().visibilityData.setVisibilityState(part.getKey(), part.getValue().getAsBoolean());
                }
            }
