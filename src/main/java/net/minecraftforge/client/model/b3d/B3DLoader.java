...
public enum B3DLoader
{
    ...
    public static final class B3DState implements ModelState, IForgeModelState
    {
        @Nullable
        ...
        public Transformation getPartTransformation(Object part)
        {
            ...
            {
                return ((IForgeModelState)parent).getPartTransformation(part).compose(nodeTransform);
            }
    ...
    private static final class ModelWrapper implements UnbakedModel, IForgeUnbakedModel
    {
    ...
    private static final class BakedWrapper implements IDynamicBakedModel
    {
        ...
        private void generateQuads(ImmutableList.Builder<BakedQuad> builder, Node<?> node, final ModelState state, ImmutableList<String> path)
        {
            ...
            if(node.getKind() instanceof Mesh && meshes.contains(node.getName()) && ((IForgeTransformation)(Object)((IForgeModelState)state).getPartTransformation(Models.getHiddenModelPart(newPath))).isIdentity())
            {
                ...
                Collection<Face> faces = mesh.bake(new Function<Node<?>, Matrix4f>()
                {
                    ...
                    private final LoadingCache<Node<?>, Transformation> localCache = CacheBuilder.newBuilder()
                        ...
                        .build(new CacheLoader<Node<?>, Transformation>()
                        {
                            ...
                            {
                                return ((IForgeModelState)state).getPartTransformation(new NodeJoint(node));
                            }
                        });
